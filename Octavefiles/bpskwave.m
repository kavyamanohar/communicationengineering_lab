...
subplot(3,1,1); plot(t,x); title('Message');axis([0 3 0 1.2]);xlabel('time(s)');ylabel('amplitude(V)');
 
%The carrier signal
y=Ac.*sin(2*pi*fc*t);
y=y/2;
subplot(3,1,2);plot(t,y); title('Carrier');xlabel('time(s)');ylabel('amplitude(V)');
 
%steps for BPSK modulation
z = x.*y;
subplot(3,1,3);plot(t,z);title('BPSK');xlabel('time(s)');ylabel('amplitude(V)');
